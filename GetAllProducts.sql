...
-- Populate price data
declare @price as table ([Style/Colour/Size ID] int PRIMARY KEY, [Selling Price 1] decimal(9,2), styleid int)
insert into @price 
	select [Style/Colour/Size ID], round(isnull([Selling Price 1],0)*1.15, 2,1), null from [Stock Detail] where [Branch ID]=@ibranch and [Selling Price 1]>0
...
update @scs set price=p.[Selling Price 1] from @scs scs, @price p where scs.id=p.[Style/Colour/Size ID] -- update per style/size/color, which doesn't cover all products
